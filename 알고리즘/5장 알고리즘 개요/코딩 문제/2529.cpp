...
단 아래 예(2 < > => 최소) 021)과 같이 첫 자리가 0인 경우도 정수에 포함되어야 한다.
...
int main() {
    ...
    vector<int> digits(10); // 0부터 9까지의 숫자
    for (int i = 0; i < 10; i++)
    {
        digits[i] = i;
    }
    ...
    do {
        ...
        for (int i = 0; i < k; i++)
        {
            if (signs[i] == '<' && digits[i] >= digits[i + 1])
            {
                valid = false;
                break;
            }
            else if (signs[i] == '>' && digits[i] <= digits[i + 1])
            {
                valid = false;
                break;
            }
        ...
        if (valid)
        {
            string numStr; // 현재 순열로 만들어진 숫자 문자열
            for (int i = 0; i <= k; i++)
            {
                numStr += to_string(digits[i]);
            }
            if (maxNum.empty() || numStr > maxNum)
            {
                maxNum = numStr;
            }
            if (minNum.empty() || numStr < minNum)
            {
                minNum = numStr;
            }
        }
    } while (next_permutation(digits.begin(), digits.end())); // 다음 순열 생성
